...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url.match(/callback/)) {
    ...
    const client_secret = await KV.findOne({
  ...
  if (req.url == "/credentials") {
    ...
    await KV.findOrCreate({
      where: { key: "spotify_client_id", value: client_id },
    });
    await KV.findOrCreate({
      where: { key: "spotify_client_secret", value: client_secret },
    });
